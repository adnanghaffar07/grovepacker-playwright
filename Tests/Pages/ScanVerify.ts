...
export class ScanVerifyPage {
  ...
  // In current use is the following function
  async observeAndVerifyProductDetailPage() {
    await this.page.waitForTimeout(2000);
    const productDetailTitle = this.page.getByText('Product Detail');
  ...
  async verifyMultiPackOrderBarCode(printLabel?: Boolean) {
    ...
    while (digits && Number(digits[0]) > 1) {
      ...
      try {
        ...
        // Prevents failure if API returns 200 OK but empty body
      }
      if (printLabel) break;
    }
  }
