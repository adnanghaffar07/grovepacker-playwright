...
test.describe(`Page - GPX Post Scanning V2`, () => {
  ...
  test('CC V2 - Test Case 01a - Serial Record with Post Scanning & Direct print - gpx-postscanning-01a', async ({
    page,
    request,
  }) => {
  ...
  test('CC V2 - Test Case 01b - Type-in-count with Post Scanning & Direct print - gpx-postscanning-01b', async ({
    ...
    await test.step('Enter valid "UPC" in "Ready For Product Scan" field and Press Enter', async () => {
      ...
      // Wait for error message to stabilize
      await page.waitForTimeout(2000);

      // Explicitly focus, clear, and fill the correct quantity
      await inputElement.click();
      await inputElement.clear();
      await inputElement.fill('9');

      // Use the Enter button from V3 locators for reliable submission
      const enterButton = scanVerifyV3Page.locators.clickOnEnterButton;
      await expect(enterButton).toBeVisible();
      await enterButton.click();

      // We must scan the product one last time to complete the order.
      await expect(inputElement).toBeHidden(); 
      await page.waitForTimeout(1000); // Wait for main screen transition
    ...
    // Verify Shipping Label appears and process it
